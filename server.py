...
#    top_five_score = [suggestion[0] for suggestion in suggestions[:5]]
    my_score = 0
    try:
        my_score = score(w2v.wv.n_similarity([word], context_words), word, candidate)
    except:
        my_score = 0
    top_five_suggestions = [suggestion[1] for suggestion in suggestions[:100] if suggestion[0] > 2.0 * my_score]
    if len(top_five_suggestions) == 0:
        return []
...
def temp(data):
    my_suggestions = []
    response = {}
    escape_words = ['করে', 'যায়']
    if data['word'] in escape_words:
